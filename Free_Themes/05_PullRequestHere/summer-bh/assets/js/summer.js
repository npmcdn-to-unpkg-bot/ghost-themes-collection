    ...
    indexPostAnimate = function () {
        if ($('.animated-post').length && $('.post-loop-animations').length) {
            ...
            var topOfWindow = $(window).scrollTop(),
                ...
                if (postPos < topOfWindow + (windowHeight/ 1.3)) {
    ...
    positionIndicator = function () {
        if ($('.js-post-reading-time').is(':visible')) {
            imagesLoaded('.summer-post-content', function () {
                var getMax = function() {
                    return $('.summer-post-content').outerHeight(true);
                };
                ...
                if('max' in document.createElement('progress')){
                    ...
                    $(document).on('scroll', function(){
                        ...
                        var authorOffsetTop = $('.summer-author-info').offset().top;
                        var stickyBottom = $('.sticky-sidebar').offset().top + $('.sticky-sidebar').outerHeight(true) + 150;

                        if (authorOffsetTop < stickyBottom) {
